...
YOUTUBE_STEP_DELAY_SECONDS = 6
YOUTUBE_AUDIO_DELAY_SECONDS = 10
...
st.markdown(
    '<div class="app-subtitle">保守加速模式：保留低頻率請求與 429/403 立即停止機制，但正常等待時間約減半。</div>',
    unsafe_allow_html=True,
)
...
with col2:
    st.caption("保守加速順序：字幕 API → 短暫等待 → 低頻率 yt-dlp →（手動允許才會）Whisper；429/403 立即停。")
