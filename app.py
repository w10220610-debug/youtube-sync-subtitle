...
def get_optional_secret(name: str) -> str | None:
    """安全讀取 Streamlit Secrets；沒有設定時直接略過。"""
    try:
...
def get_ytdlp_common_options(
    ...
    """使用 yt-dlp 現行預設客戶端，並支援 cookies、代理與自動 PO Token provider。"""
    opts = {
        "quiet": True,
        "no_warnings": True,
        "noplaylist": True,
    ...
    # 不再硬塞過時的 android/web/tv 組合；預設讓新版 yt-dlp 自行選擇。
    if player_clients:
        opts["extractor_args"] = {
            "youtube": {
                "player_client": player_clients,
                # 若 requirements 已安裝 PO Token provider，yt-dlp 會自動取 token。
                "fetch_pot": ["auto"],
            }
        }
...
def download_transcript_api(
    ...
    preferred = CHINESE_CODES + ENGLISH_CODES
    selected = None
    ...
    fetched = selected.fetch().to_raw_data()
    subtitles = normalize_subtitles(fetched)
    if not subtitles:
        return None

    language_code = str(getattr(selected, "language_code", "")).lower()
    is_chinese = language_code.startswith("zh")
...
def download_youtube_subtitles(
    ...
    for attempt_name, clients in attempts:
        ...
        try:
            ...
            last_error = exc
            # 遇到 429 不連續轟炸同一共享 IP，只短暫停一次後切換策略。
            if "429" in str(exc):
                time.sleep(2)
            continue
...
def load_subtitles_with_fallback(
    ...
    with tempfile.TemporaryDirectory(prefix="yt_caption_") as tmp:
        ...
        if not allow_speech_recognition:
            raise RuntimeError("找不到可用字幕，而且你沒有開啟語音辨識。可開啟語音辨識，或在專案根目錄加入 cookies.txt 後重試。")
    ...
    raise RuntimeError("所有字幕取得方式都失敗。雲端共享 IP 可能已被 YouTube 限制。最高成功率做法是設定住宅輪替代理；登入限定影片則另外上傳 cookies.txt。\n\n" + "\n".join(errors))
...
@media (max-width: 850px) and (orientation: portrait) {{
    .grid {{
        ...
        grid-template-rows: auto minmax(320px, 46vh);
        height: auto;
        gap: 12px;
    }}
    .transcript-panel {{
