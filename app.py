...
YOUTUBE_STEP_DELAY_SECONDS = 2
YOUTUBE_AUDIO_DELAY_SECONDS = 4
...
def contains_cjk(text: str) -> bool:
    """判斷文字是否包含中文/CJK 字元。"""
    return bool(re.search(r"[\u3400-\u4dbf\u4e00-\u9fff]", clean_text(text)))


def looks_like_untranslated_english(source_text: str, translated_text: str) -> bool:
    """偵測翻譯結果是否仍像完整英文句子。"""
    source = clean_text(source_text)
    candidate = clean_text(translated_text)

    if not source:
        return False

    if not candidate or is_translation_error_response(candidate):
        return True

    if contains_cjk(candidate):
        return False

    english_words = re.findall(r"[A-Za-z]+(?:'[A-Za-z]+)?", source)
    candidate_words = re.findall(r"[A-Za-z]+(?:'[A-Za-z]+)?", candidate)

    # 至少 3 個英文單字才視為「整句英文」，避免 Amazon / BMW 這類專有名詞誤判
    if len(english_words) >= 3:
        ...
        if (
            alpha_count > 0
            and ascii_alpha_count / alpha_count >= 0.85
...
def translate_texts_to_chinese(
    ...
    """
    第一輪正常翻譯。
    完成後自動掃描仍像完整英文的字幕，只針對漏翻句進行第二輪補翻。
    """
    translated: list[str] = []
    total = max(len(texts), 1)

    stage_span = max(progress_end - progress_start, 0.0)
    first_pass_end = progress_start + stage_span * 0.88

    update_work_progress(
        progress,
        current_task,
        progress_start,
        f"{stage_name}｜第一輪，共 {len(texts)} 段",
    )
    ...
    for index, text in enumerate(texts):
        ...
        ratio = (index + 1) / total
        mapped = progress_start + (first_pass_end - progress_start) * ratio
        update_work_progress(
            progress,
            current_task,
            mapped,
            f"{stage_name}｜第一輪 {index + 1}/{total}",
        )

    # 掃描漏翻
    pending = [
        index
        for index, (source, candidate) in enumerate(zip(texts, translated))
        if looks_like_untranslated_english(source, candidate)
    ]

    if not pending:
        st.session_state["last_untranslated_count"] = 0
        update_work_progress(
            progress,
            current_task,
            progress_end,
            f"{stage_name}｜完成，未發現漏翻",
        )
        return translated
    ...
    update_work_progress(
        ...
        current_task,
        first_pass_end,
        f"{stage_name}｜發現 {len(pending)} 段可能漏翻，準備補翻",
    )

    # 上一版補翻前等 2 秒，這版縮短為 1 秒
    time.sleep(1)

    rescued = 0
    pending_total = len(pending)

    for pending_pos, index in enumerate(pending, start=1):
        source_text = clean_text(texts[index])
        repaired = translated[index]

        # 只補翻漏掉的句子；固定 en -> zh-TW
        for attempt in range(2):
            try:
                rescue_translator = GoogleTranslator(source="en", target="zh-TW")
                candidate = clean_text(rescue_translator.translate(source_text))

                if (
                    candidate
                    and not is_translation_error_response(candidate)
                    and not looks_like_untranslated_english(source_text, candidate)
                ):
                    repaired = candidate
                    rescued += 1
                    break
        ...
        ratio = pending_pos / max(pending_total, 1)
        mapped = first_pass_end + (progress_end - first_pass_end) * ratio
        update_work_progress(
            progress,
            current_task,
            mapped,
            f"{stage_name}｜補翻漏翻字幕 {pending_pos}/{pending_total}",
        )

    leftovers = [
        index
        for index in pending
        if looks_like_untranslated_english(texts[index], translated[index])
    ]

    st.session_state["last_untranslated_count"] = len(leftovers)

    if leftovers:
        st.warning(
            f"⚠️ 已自動補翻 {rescued} 段，但仍有 {len(leftovers)} 段未成功翻成中文；"
            "這些字幕先保留英文原文，避免把錯誤頁文字當成字幕。"
        )
        try:
            st.toast(
                f"仍有 {len(leftovers)} 段字幕未翻譯成功",
                icon="⚠️",
            )
        except Exception:
            pass
        ...
        update_work_progress(
            ...
            current_task,
            progress_end,
            f"{stage_name}｜完成，但仍有 {len(leftovers)} 段保留英文",
        )
    else:
        update_work_progress(
            progress,
            current_task,
            progress_end,
            f"{stage_name}｜補翻完成，修復 {rescued} 段",
        )
...
with col2:
    st.caption("加速保守模式＋即時工作進度：正常流程再小幅加速，並自動補翻漏掉的英文字幕；429/403 仍會立即停止並彈出通知。")
...
if load_button:
    ...
    if not video_id:
        ...
        try:
            ...
            untranslated_count = int(st.session_state.get("last_untranslated_count", 0) or 0)

            if untranslated_count > 0:
                current_task.warning(
                    f"⚠️ **目前執行：處理完成，但仍有 {untranslated_count} 段保留英文原文。**"
                )
                status.update(
                    label=f"⚠️ 中文字幕完成｜仍有 {untranslated_count} 段未翻譯",
                    state="complete",
                    expanded=True,
                )
            else:
