...
def get_ytdlp_common_options() -> dict:
    """提高 YouTube 雲端環境擷取成功率，並在有 cookies.txt 時自動使用。"""
    opts = {
        "quiet": True,
        "no_warnings": True,
        "noplaylist": True,
...
def load_subtitles_with_fallback(
    ...
    with tempfile.TemporaryDirectory(prefix="yt_caption_") as tmp:
        ...
        status.write("① 正在透過 yt-dlp 尋找人工字幕與自動字幕……")
        try:
    ...
    raise RuntimeError("所有字幕取得方式都失敗。Streamlit Cloud 的共享 IP 可能被 YouTube 限制；可在專案根目錄加入 cookies.txt，或改成本機執行。\n\n" + "\n".join(errors))
...
st.markdown(
    '<div class="app-subtitle">優先抓取 YouTube 人工／自動字幕；抓不到時，自動改用 Whisper 語音辨識。</div>',
    unsafe_allow_html=True,
)
...
with col2:
    st.caption("處理順序：yt-dlp 人工／自動字幕 → Whisper 語音辨識。")
